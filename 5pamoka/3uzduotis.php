...
$FilteredData = dataFilter($dates);

var_dump($FilteredData);


function dataFilter($TimeData){
    ...
    foreach ($TimeData as $index => $val){

        $another_date = new DateTime($val, new DateTimeZone('UTC'));
        $difference = $another_date->diff($current_date);

        if($difference->y <= 5){

            $filteredDate[$index]= $another_date;
